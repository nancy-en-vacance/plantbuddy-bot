...
async def water(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [
        [InlineKeyboardButton(str(row["name"]), callback_data=_pid(row["plant_id"]))]
...
async def watered(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    mask = df["plant_id"].apply(_match_pid)

    if not mask.any():
        await query.edit_message_text("⚠️ I couldn't find that plant in the sheet.")
        return
...
def main():
    ...
    base_url = os.environ["BASE_URL"].rstrip("/")  # e.g. https://plantbuddy-bot.onrender.com
